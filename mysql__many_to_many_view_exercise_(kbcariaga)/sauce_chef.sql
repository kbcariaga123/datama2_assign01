...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 06, 2019 at 05:22 PM
...
--
-- Stand-in structure for view `ingredients_food`
-- (See below for the actual view)
--
CREATE TABLE `ingredients_food` (
`ingredient_code` varchar(45)
,`ingredient_desc` varchar(45)
...
--
-- Structure for view `ingredients_food`
--
DROP TABLE IF EXISTS `ingredients_food`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `ingredients_food`  AS  select `ingredients`.`ingredient_code` AS `ingredient_code`,`ingredients`.`ingredient_desc` AS `ingredient_desc`,`food`.`food_name` AS `food_name`,`food`.`food_price` AS `food_price` from ((`ingredients` join `recipe` on(`ingredients`.`id` = `recipe`.`ingredients_id`)) join `food` on(`food`.`id` = `recipe`.`food_id`)) ;
